    ...
    <div class="col-md-8">
        <div class="card card-admin">
            <div class="card-header">
                <h4 class="mb-0"><i class="fas fa-plus-circle mr-2"></i>Dodaj Nowy Kurs</h4>
            </div>
            <div class="card-body">
                @if ($errors->any())
                ...
                <form action="{{ route('admin.courses.store') }}" method="POST">
                    ...
                    <div class="form-group">
                        ...
                        <select class="form-control form-control-admin" id="level" name="level" required>
                            <option value="" disabled {{ old('level') ? '' : 'selected' }}>Wybierz poziom</option>
                            @php
                    ...
                    <div class="form-group">
                        ...
                        <select class="form-control form-control-admin" id="instructor_id" name="instructor_id" required>
                            ...
                            @foreach ($instructors as $id => $name) {{-- Zakładając, że $instructors to tablica [id => name] --}}
                                <option value="{{ $id }}" {{ old('instructor_id') == $id ? 'selected' : '' }}>{{ $name }}</option>
                            @endforeach
                        </select>
                    </div>

                    <button type="submit" class="btn btn-admin-primary"><i class="fas fa-save mr-1"></i> Dodaj Kurs</button>
