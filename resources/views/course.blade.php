...
<body>
    ...
    <div class="container mt-4">
        <div class="row">
            ...
            <div class="d-flex flex-wrap align-items-start justify-content-between">
                <div style="flex:1 1 340px; min-width:260px;">
                    ...
                    <div class="row mb-3">
                        <div class="col-md-6 mb-2">
                            <div class="mb-2"><i class="fas fa-language mr-2 text-secondary"></i><strong>Język:</strong> {{ $course->language }}</div>
                            <div class="mb-2"><i class="fas fa-signal mr-2 text-secondary"></i><strong>Poziom:</strong> {{ $course->level }}</div>
                            <div class="mb-2"><i class="fas fa-calendar-plus mr-2 text-secondary"></i><strong>Start:</strong> {{ $course->start_date }} <strong>Koniec:</strong> {{ $course->end_date }}</div>
                        </div>
                        <div class="col-md-6 mb-2">
                            <div class="mb-2"><i class="fas fa-users mr-2 text-secondary"></i><strong>Miejsca:</strong> {{ $course->group_size - count($course->enrollments) }}/{{ $course->group_size }}</div>
                    ...
                    <div class="d-flex align-items-center mb-3">
                        @if(!Auth::user())
                            <a href="{{ route('login') }}" class="btn btn-primary btn-lg rounded-pill px-4 shadow-sm">Zaloguj się aby zapisać się na kurs</a>
                        @elseif ($course->enrollments->contains('user_id', Auth::id()))
                            <button class="btn btn-secondary btn-lg rounded-pill px-4" disabled>Jesteś już zapisany</button>
                        @elseif (count($course->enrollments) >= ($course->group_size))
                            <button class="btn btn-secondary btn-lg rounded-pill px-4" disabled>Brak wolnych miejsc</button>
                        @elseif($course->start_date < now())
                            <button class="btn btn-secondary btn-lg rounded-pill px-4" disabled>Kurs został zakończony</button>
                        @elseif(Auth::user())
                            <a href="{{ route('course.enrollUser', $course->id) }}" class="btn btn-primary btn-lg rounded-pill px-4 shadow-sm">Zapisz się na kurs</a>
                        @endif
                        <a href="{{ url('/main') }}" class="btn btn-outline-primary rounded-pill px-4">Powrót do kursów</a>
                    </div>
                </div>
                <div class="instructor-box ml-md-4 mt-4 mt-md-0 shadow-sm" style="background:#f7f7fa; border:1px solid #ececec; min-width:220px; max-width:300px;">
                    <h5 class="mb-2" style="font-weight:600; color:var(--main-color);"><i class="fas fa-chalkboard-teacher mr-2"></i>Instruktor</h5>
        ...
        <!-- Sekcja lekcji na całą szerokość -->
        <div class="row">
            <div class="col-12">
                <div class="card mt-4">
                                ...
                                <li class="list-group-item">
                                    <div class="lesson-item">
                                        <div>
                                            ...
                                            <small>{{ $lesson->date }} o {{ $lesson->time }}</small>
                                        </div>
        ...
        <div class="row mt-4">
            <div class="col-md-3">
                ...
                <div class="card h-100">
                    <div class="card-header">
                        <h5 class="mb-0"><i class="fas fa-pen mr-2"></i>Dodaj swoją opinię</h5>
                    </div>
                    <div class="card-body">
            ...
            <div class="col-md-9">
                ...
                <div class="card h-100">
                    <div class="card-header">
                        <h5 class="mb-0"><i class="fas fa-comments mr-2"></i>Opinie innych użytkowników</h5>
                    </div>
